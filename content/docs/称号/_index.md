...
- [スタンダードクラスのカード150種類](docs/ライブラリ#e2e7bbf9)をすべて集めると出現する
- ただし、ブラザーバンドによるライブラリ共有でライブラリを埋めてもコンプリートとは見なされない<br />
  （セーブ画面で出てくる数字が実際に入手した種類数）

### メガクラスコンプリートの称号

- [メガクラスのカード42種類](docs/ライブラリ#w14d211b)をすべて集めると出現する
- ブラザーとのライブラリ共有についてはスタンダードと同様


### ギガクラスコンプリートの称号

- [ギガクラスのカード5種類](docs/ライブラリ#j6e21f7b)をすべて集めると出現する
- ブラザーとのライブラリ共有についてはスタンダードと同様
- 配信ギガカードはコンプリートに必要ない。
...
- 次元の狭間2にて、[アポロン・フレイム](docs/ボスキャラ#t8441453)（無印）を倒すと出現する
